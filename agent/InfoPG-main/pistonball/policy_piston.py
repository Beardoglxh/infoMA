...
class PistonPolicy(BasePolicy):
    def __init__(self, encoding_size, policy_latent_size, action_space_n, device, adv_type, model_state_dict=None,
                 constant=False):
        ...
        if model_state_dict is not None:
            self.policy.load_state_dict(model_state_dict)  # TODO 这里可以读取模型参数
        self.policy.to(device)


class Piston_PolicyHelper(nn.Module):
    def __init__(self, encoding_size, policy_latent_size,
                 action_space_n):  # TODO encoding_size和policy_latent_size在最开始的参数有定义
        super().__init__()
