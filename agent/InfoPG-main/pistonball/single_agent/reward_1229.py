...
class Reward:
    ...
    def __call__(self, my_planes, bait_planes, enemy_planes):
        ...
        if pca_enemy_planes[0].X >= 10000:  # 未达到巡逻范围
            self.angle_reward(bait_planes)
        else:
    ...
    def distance_reward(self, bait_planes, filt_planes):
        distance = self.get_dis(bait_planes[0].pos3d, filt_planes[0].pos3d)
        if distance < 25000:
            self.reward_dict[bait_planes[0].ID] += 4
