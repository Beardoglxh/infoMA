...
class BatchEnv(ABC):
    def __init__(self, env_name: str, batch: int, env_params: Dict, path=None, seed=None):
        ...
        if env_name == 'pistonball_v4':  # TODO 标记了obs_shape, 但这个reward_scale不知道什么意思
            self.envs = [pistonball_v4.parallel_env(**env_params) for i in range(0, self.BATCH_SIZE)]  # 这里是并行了四个环境, 这里的话xsim其实也可以, 先按照一个来吧..
            if seed is not None:
        ...
        if path is not None:  # TODO 如果要存储的话可能要改这里
            self.path = path
            self.save = True
    ...
    def batch_reset(self):
        ...
        ret_obs = {agent_name: np.zeros((self.BATCH_SIZE,) + self.OBS_SHAPE) for agent_name in
                   self.AGENT_NAMES}  # TODO obs的形状在这里加上了batch_size
        for batch_ix in range(0, self.BATCH_SIZE):
            obs = self.envs[batch_ix].reset()  # TODO 在这里得到了观测值?
    ...
    @abstractmethod
    def loop(self, user_params: Dict, policies: Dict[str, BasePolicy], optimizers: Dict[str, optim.Optimizer],
             schedulers):
        pass
    ...
    def batch_step(self, actions: Dict[str, List], step_num: int, time_penalty: float, early_reward_benefit: float):
        ...
        for batch_ix in range(0, num_left_batches):
            ...
            action_dict = {agent: batched_action[batch_ix] for agent, batched_action in actions.items()}
            obs_temp, rewards_temp, dones_temp, _ = self.envs[actual_batch_ix].step(action_dict)  # TODO step函数可能需要重写
