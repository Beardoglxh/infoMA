...
from create_xsim_env import CreateXsim
...
def test_piston_with_hyperparams_with_xsim(hyper_params, verbose):
    ...
    action_space = hyper_params['action_space']  # TODO 这里定义了action_space
    ...
    env = CreateXsim(batch_size, env_params)
    if hyper_params['transfer_experiment'] is not None:  # 表示模型中是否进行迁移学习实验，也就是是否使用已经训练好的模型来适应新的环境或任务
        policies, optimizers = create_policies_from_experiment(hyper_params, device)
    else:
        policies = {agent: PistonPolicy(encoding_size, policy_latent_size, action_space, device, adv_type) for agent in
                    env.get_agent_names()}
    ...
    if hyper_params['scheduler'] is not None:
        if hyper_params['scheduler']['type'] == 'MultiStepLR':
            print('*Using MultiStepLR with milestones at: %s and gamma factor: %s' % (
                hyper_params['scheduler']['params']['milestones'],
                hyper_params['scheduler']['params']['gamma']))
            schedulers = {agent: optim.lr_scheduler.MultiStepLR(optimizer=optimizers[agent],
                                                                **hyper_params['scheduler']['params']) for agent in
                          env.get_agent_names()}
        else:
...
def create_policies_from_experiment(hyper_params, device):
    ...
    policies = {}
    optimizers = {}
    action_space = hyper_params['action_space']
    for i in range(0, len(piston_order)):
        ...
        policies['红无人机%s' % (2 * i + 1)] = PistonPolicy(encoding_size, policy_latent_size, action_space, device,
                                                        'normal',
                                                        model_state_dict=data['policy'])  # TODO 名字这里取巧了, 后面还得改
        optimizers['红无人机%s' % (2 * i + 1)] = optim.Adam(policies['红无人机%s' % (2 * i + 1)].parameters(), lr)
        optimizers['红无人机%s' % (2 * i + 1)].load_state_dict(data['optimizer'])
