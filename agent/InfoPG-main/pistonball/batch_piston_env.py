...
class PistonEnv(BatchEnv):
    ...
    def loop(self, user_params: Dict, policies: Dict[str, PistonPolicy], optimizers: Dict[str, torch.optim.Optimizer],
             schedulers):
        """
...
class PistonEnv_MOA(BatchEnv):
    ...
    def loop(self, user_params: Dict, policies: Dict[str, MOAPolicy], optimizers: Dict[str, torch.optim.Optimizer],
             schedulers):
        device = user_params['device']
        alex_encoder = Encoder(device)
        epochs = user_params['epochs']
            ...
            for step in range(self.MAX_CYCLES):
                ...
                for agent_ix, agent in enumerate(self.AGENT_NAMES):
                    ...
                    for batch_ix in range(0, num_left_batches):
                        ...
                        input_messages.append(prev_action_dists[actual_batch_ix, neighbor_mask].unsqueeze(
                            0))  # -> appending 1, seq_len (neighbors), 3
                    input_prev_actions = torch.cat(input_messages, dim=0).to(device)
                    output_dist, output_value, output_actions_neighbors = policies[agent].forward(observations[agent],
                                                                                                  None,
                                                                                                  input_prev_actions)
                ...
                for agent_ix, agent in enumerate(self.AGENT_NAMES):
                    ...
                    for batch_ix in range(0, num_left_batches):
                        ...
                        for neighbor_ix, neighbor in enumerate(neighbor_names):
                            actual_output_dist[batch_ix, neighbor_ix] = actions[neighbor][
                                batch_ix]  # action taken by some agent==neighbor_ix, and at some batch_ix

                    for batch_ix in range(0, num_left_batches):
                        actual_batch_number = left_batches[batch_ix]
                        memory[agent][actual_batch_number].regularizer = loss(info[agent][2][batch_ix],
                                                                              actual_output_dist[batch_ix])
...
if __name__ == '__main__':
    ...
    policies = {agent: MOAPolicy(encoding_size, policy_latent_size, action_space, device) for agent in
                env.get_agent_names()}
    optimizers = {agent: optim.Adam(policies[agent].parameters(), lr) for agent in env.get_agent_names()}
    env.loop(user_params, policies, optimizers, None)
